...
import net.minekingdom.continuum.WorldConfiguration;
...
public class ContinuumWorld {
	...
	private ContinuumDimension spawnDimension;
	
	private Map<String, ContinuumDimension> dimensions;
	
	private WorldConfiguration config;
	
	public ContinuumWorld(Server server, String name) {
		this.server = server;
		this.name = name;
		this.dimensions = new HashMap<String, ContinuumDimension>();
		this.config = new WorldConfiguration(this);
		
		load();
	}
	
	public ContinuumWorld(Server server, String name, WorldCreator creator, String generator) {
		this.server = server;
		this.name = name;
		this.dimensions = new HashMap<String, ContinuumDimension>();
		this.config = new WorldConfiguration(this);
	...
	public void load(WorldCreator creator, String gen) {
		...
		for (ContinuumDimension dim : this.dimensions.values()) {
			dim.unload();
		}
		...
		for (Map.Entry<String, ConfigurationSection> dim : dimensionConfig.entrySet()) {
			...
			ConfigurationSection dc = dim.getValue();
				...
				WorldType type = WorldType.valueOf(dc.getString("world-type", "normal").toUpperCase());
			
			ContinuumDimension cd;
		...
		ContinuumDimension spawnDimension = this.dimensions.get(dimensionName);
	...
	public void save() {
		...
		ConfigurationSection world = config.getWorldConfigSection();
		for (ContinuumDimension dim : dimensions.values()) {
			ConfigurationSection section = world.createSection(dim.getName());
	...
	public ContinuumWorld addDimension(ContinuumDimension dimension) {
	...
	public ContinuumWorld removeDimension(ContinuumDimension dimension) {
	...
	public Collection<ContinuumDimension> getDimensions() {
		return this.dimensions.values();
	}
	
	public static class DefaultDimension extends ContinuumDimension {

		public DefaultDimension(Server server, ContinuumWorld parent) {
			super(server, parent, "surface");
			this.keepSpawnInMemory = true;
		}
		
		public DefaultDimension(Server server, ContinuumWorld world, long seed, String generator, Environment environment, WorldType type) {
			super(server, world, "surface", seed, generator, environment, type);
			this.keepSpawnInMemory = true;
		}

		@Override
		protected WorldCreator getWorldCreator() {
			return new WorldCreator(world.getName())
				.seed(seed)
	...
	public static ContinuumWorld get(World world) {
		if (world != null) {
			List<MetadataValue> meta = world.getMetadata("continuum.world");
			if (meta.size() > 0 && meta.get(0).value() instanceof ContinuumWorld) {
				return (ContinuumWorld) meta.get(0).value();
			}
		}
		return null;
	}

	public ContinuumDimension getDimension(String name) {
